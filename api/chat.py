...
from ai.voice_module import speak, tts_to_mp3_bytes, tts_to_wav_bytes

chat_bp = Blueprint("chat", __name__)


def _build_tts_payload(text: str):
    audio_b64 = ""
    audio_format = ""
    clean = (text or "").strip()
    if not clean:
    ...
    mp3_bytes = tts_to_mp3_bytes(clean)
    if mp3_bytes:
        return base64.b64encode(mp3_bytes).decode("ascii"), "mp3"

    wav_bytes = tts_to_wav_bytes(clean)
    if wav_bytes:
...
def api_chat():
    ...
    try:
        ...
        audio_b64, audio_format = _build_tts_payload(reply)
        return jsonify(
            {
                "ok": True,
                "reply": reply,
