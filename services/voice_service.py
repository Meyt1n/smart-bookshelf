...
def process_text_remote(text: str, image_bytes=None, push_events=True):
    ...
    if intent == "take":
        shelf_result = take_by_text(normalized, speak_out=False)
        ok, msg, ai_reply = shelf_result
        reply = getattr(shelf_result, "reply", None) or ai_reply or ""
        if push_events:
            if msg:
...
def process_text_model_dispatch(text: str, image_bytes=None, push_events=True):
    ...
    reply = exec_result.get("reply") or exec_result.get("ai_reply") or response_text or ""
    result = {
        "ok": bool(exec_result.get("ok", True)),
        "intent": exec_result.get("intent"),
        "text": normalized,
        "reply": reply,
        "ai_reply": exec_result.get("ai_reply"),
        "msg": exec_result.get("msg"),
        "need_image": exec_result.get("need_image"),
        "picked": exec_result.get("picked"),
        "dispatch_request": exec_result.get("dispatch_request"),
        "commit_request": exec_result.get("commit_request"),
        "commands": commands,
        "model_raw": raw_reply,
    }
    return result
